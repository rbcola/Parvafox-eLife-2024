###################################################
#T-Test analysis of Opto hot plate JumpOrCorrecLick##
###################################################
...
#create outputdir
output_dir=paste(getwd(),"/JumpOrCorrectLick/", sep="")
...
#read data
myData=read.xlsx("D:/Reto (old harddrive)/UNIFR/MSc.EBR/Masterarbeit/HotPlate/22-12-21_Opto_HotPlate_allAnimals/22-09-07_Opto_HotPlate_JumpOrCorrectLick.xlsx")
...
for (qq in unique(wideData$Genotype)){
  ...
  for (jj in unique(qqData$Opsin)){
    ...
    nam=paste(output_dir,"Opto_JumpOrCorrectLick_",qq,"_",jj,"_paired_tTest-Wilcoxon",".doc",sep="")
  ...
  gg <-  ggplot(long_qqData, aes(x=`Condition`, y=`Value`, fill=Condition)) +
    ...
    labs(title = "Opto Hot Plate Jump or Correct Lick", x=element_blank(), y="Latency to endpoint (s)")+
    theme(plot.title = element_text(
      hjust = 0.5,
      vjust = 0.5,
      face = 'bold'))
  ggsave(paste(output_dir,"Opto_HotPlate_JumpOrCorrectLick_DotPlot-Boxplot_",qq,".tiff"), height=5, width=7)
  ...
  gg_color <-  ggplot(long_qqData, aes(x=`Condition`, y=`Value`, fill=Condition)) +
    ...
    theme(plot.title = element_text(
      ...
      vjust = 0.5,
      face = 'bold'))
  ggsave(paste(output_dir,"ColorCoded_Opto_HotPlate_JumpOrCorrectLick_DotPlot-Boxplot_",qq,".tiff"), height=5, width=7)
  
}
#reorder columns in Results DF and then save it as xlsx
Results <- Results[,c(1,7,2,3,4,5,6,8)]
ResNam=paste(output_dir,"Opto_HotPlate_JumpOrCorrectLick_tTest_overview",".xlsx",sep="")
