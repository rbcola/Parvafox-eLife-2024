################################################
##### T-Test analysis of Opto hot plate trylick#
################################################
...
#create outputdir
output_dir=paste(getwd(),"/1stLickAttempt_correctEP/", sep="")
...
#read data
myData=read.xlsx("D:/Reto (old harddrive)/UNIFR/MSc.EBR/Masterarbeit/HotPlate/22-12-21_Opto_HotPlate_allAnimals/22-09-07_Opto_HotPlate_1stLickAttempt_or_correctEP.xlsx")
...
for (qq in unique(wideData$Genotype)){
  ...
  for (jj in unique(qqData$Opsin)){
    ...
    if( normality["normtest.p"]> 0.05 & normality["skew.2SE"]<1 & normality["kurt.2SE"]<1){
      ...
      Results[nrow(Results),6]=EffSize$estimate
      Results[nrow(Results),7]=qq
      Results[nrow(Results),8]=Results[nrow(Results),8]=levels(EffSize$magnitude)[EffSize$magnitude]
    }
    
    #save inividual hypothesis test output
    nam=paste(output_dir,"Opto_1stLickAttempt-correctEP_",qq,"_",jj,"_paired_tTest-Wilcoxon",".doc",sep="")
  ...
  gg <-  ggplot(long_qqData, aes(x=`Condition`, y=`Value`)) +
    ...
    geom_point(aes(group = Animal, color = Animal), size = 3)+
    facet_wrap(~ Opsin)+
    theme_bw()+
    #stat_summary(fun.y=median, geom="point", shape=18, size=3, color="blue", position=position_dodge(0.5))+
    labs(title = "Opto Hot Plate 1st Lick Attempt\n or Correct Endpoint", x=element_blank(), y="Latency to behavior (sec)")+
    theme(plot.title = element_text(
      hjust = 0.5,
      vjust = 0.5,
      face = 'bold'))
  ggsave(paste(output_dir,"Opto_HotPlate_1stLickAttempt-correctEP_DotPlot-Boxplot_",qq,".tiff"), height=4, width=3)

}
#reorder columns in Results DF and then save it as xlsx
Results <- Results[,c(1,7,2,3,4,5,6,8)]
ResNam=paste(output_dir,"Opto_HotPlate_1stLickAttempt-correctEP_tTest_overview",".xlsx",sep="")
