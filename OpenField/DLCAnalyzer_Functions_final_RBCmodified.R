...
PlotDensityPathsMyColors <- function(t,points,SDcutoff = 4, Title = "density path"){
  if(!IsTrackingData(t)){
    stop("Object is not of type TrackingData")
  }
  out <- NULL
  ...
  for(i in points){
    ...
    out[[i]] <- ggplot(data = data_plot, aes(x,y)) + 
      stat_density_2d(data = data_plot, aes(latbin,longbin, fill=..density..), geom = "raster", contour = FALSE) + 
      scale_fill_scico(name = "Time Density", palette = "romaO") +
      geom_path(data = data_plot, aes(x,y, color = SDPlot((speed * t$fps),SDcutoff))) + 
      theme_bw() + 
      ggtitle(paste(i,Title, sep = " ")) + 
      scale_color_gradient(name = paste("speed (",t$distance.units,"/s)",sep = ""), high = "gray", low="magenta")
  }
  return(out)
}

PlotDensityPaths_binned <- function(t, points, SDcutoff = 4, Title = "density path"){
  if(!IsTrackingData(t)){
    stop("Object is not of type TrackingData")
  }
  ...
  for(i in points){
    ...
    for(j in 1:nrow(t$bins)){
      tbin <- CutTrackingData(t, keep.frames = t$bins[j,"from"]:t$bins[j,"to"])
      #tb$data$Occiput$frame <- tb$data$Occiput$frame - tb$data$Occiput$frame[1]+1
    
      data_plot <- tbin$data[[i]]
      data_scale <- t$data[[i]]
      xbreaks <- seq(floor(min(data_scale$x)), ceiling(max(data_scale$x)), by = 0.1)
      ybreaks <- seq(floor(min(data_scale$y)), ceiling(max(data_scale$y)), by = 0.1)
      data_plot$latbin <- xbreaks[cut(data_plot$x, breaks = xbreaks, labels=F)]
      data_plot$longbin <- ybreaks[cut(data_plot$y, breaks = ybreaks, labels=F)]
      
      plots[[i]][[t$bins$bin[j]]] <- ggplot(data = data_plot, aes(x,y)) + 
        stat_density_2d(data = data_plot, aes(latbin,longbin, fill = ..density..), geom = "raster", contour = FALSE) + 
        scale_fill_scico(name = "Time Density", palette = "romaO") +
        xlim(floor(min(data_scale$x)), ceiling(max(data_scale$x)))+
        ylim(floor(min(data_scale$y)), ceiling(max(data_scale$y)))+
        geom_path(data = data_plot, aes(x,y, color = SDPlot((speed * t$fps),SDcutoff))) + 
        theme_bw() + 
        ggtitle(paste(i,Title, sep = " ")) + 
        scale_color_gradient(name = paste("speed (",t$distance.units,"/s)",sep = ""), high = "gray", low="magenta")
...
FreezingAnalysis <- function(t, cutoff_freezing, integration_period = 0, points, Object){
  if(!IsTrackingData(t)){
    stop("Object is not of type TrackingData")
  }
  if(sum(t$point.info$PointType == Object) == 0){
  ...
  t$object <- list()
  t$object$movement <- abs(apply(acc,1,FUN = mean))
  t$object$is.freezing <- avgmean(t$object$movement,integration_period) < cutoff_freezing
  t$labels$cutoff.freezing <- ifelse(t$object$is.freezing == 1, "Freezing","None")
  
  t$Report <- list()
  t$Report[["time.freezing"]] <- sum(t$object$is.freezing) / t$fps
  t$Report[["total.time"]] <- length(t$object$is.freezing) / t$fps
  t$Report[["percentage.freezing"]] <- sum(t$object$is.freezing) / length(t$object$is.freezing) * 100
  ...
  for(k in points){
    ...
    t$Report[[paste(k, "raw.distance", sep = ".")]] <- sum(t$data[[k]]$speed, na.rm = T)
    t$Report[[paste(k, "raw.speed", sep = ".")]] <- mean(t$data[[k]]$speed, na.rm = T) * t$fps
    t$Report[[paste(k, "distance.moving", sep = ".")]] <- sum(t$data[[k]]$speed[!t$object$is.freezing], na.rm = T)
    t$Report[[paste(k, "speed.moving", sep = ".")]] <- mean(t$data[[k]]$speed[!t$object$is.freezing], na.rm = T) * t$fps
  }
...
PlotZoneVisitsWithMoving <- function(t, points, zones = NULL, p.size = 2){
  if(!IsTrackingData(t)){
    stop("Object is not of type TrackingData")
  }
  if(is.null(t$zones)){
  ...
  for(j in points){
    for(i in zones){
      print
      dat <- rbind(dat,data.frame(seconds = t$seconds, zone = ifelse(IsInZone(t,j,i,t$zones.invert[[i]]),i,NA), type = "automatic", points = j, is.separate = 0))
    }
    dat <- rbind(dat,data.frame(seconds = t$seconds, zone = ifelse(t$data[[j]][["is.moving"]],"is.moving",NA), type = "automatic", points = j, is.separate = 1))
  }
  
  #create blank dataframe for making sure all factors are plotted on y scale
  blank <- data.frame(seconds = 1:length(t$zones), zone = unique(names(t$zones)), type = rep("automatic", length(t$zones)), points = rep(j, length(t$zones)), is.separate = rep(0,length(t$zones))) 

  ggplot(data = na.omit(dat),aes(seconds, zone, color = zone)) + 
    geom_point(size = p.size, shape = 124) +
    geom_blank(data = blank, aes(seconds, zone))+
    facet_grid(is.separate~., scales = "free", space = "free")+
    theme_bw()+
...
PlotDensityPathsMyColors.Multi.PDF <- function(ts, points, filename = "DensityPathMulti",width = 10, height = 8, add_zones = FALSE, selected_zones = NULL, ...){
  if(IsTrackingData(ts)){
    x <- ts
    ts <- list()
    ts[[paste(x$filename)]] <- x
  ...
  for(i in names(ts)){
    if(IsTrackingData(ts[[i]])){
      ps <- PlotDensityPathsMyColors(ts[[i]],points = points, Title = i, ...)
...
PlotDensityPaths_binned.Multi.PDF <- function(ts, points, filename = "DensityPathMulti_binned",width = 19, height = 8, add_zones = FALSE, selected_zones = NULL, ...){
  if(IsTrackingData(ts)){
    x <- ts
    ts <- list()
    ts[[paste(x$filename)]] <- x
  ...
  for(i in names(ts)){
    if(IsTrackingData(ts[[i]])){
      ps <- PlotDensityPaths_binned(ts[[i]],points = points, Title = i, ...)
      ...
      if(add_zones){
        if(!is.null(ts[[i]]$zones)){
          ...
          if(length(zones) == 0){
            ...
            for(subplot in ps){
              ps$subplot <- AddZonesToPlots(ps$subplot,ts[[i]]$zones[zones])
            }
          }
        }else{
          warning(paste("can not add zones to",i, ".Does not have zones defined", sep = " "))
  ...
  pdf(paste(filename,".pdf",sep = ""), width = width, height = height)
  for(i in out){
    grid.arrange(i[[points]]$B1, i[[points]]$B2, ncol = 2)
  }
  dev.off()
  return(NULL)
}
...
PlotZoneVisitsWithMoving.Multi.PDF <- function(ts, points,filename = "ZoneVisitsMultiWithMoving", width = 10, height = 8,...){
  if(IsTrackingData(ts)){
    x <- ts
    ts <- list()
    ts[[paste(x$filename)]] <- x
  ...
  pdf(paste(filename,".pdf",sep = ""), width = width, height = height)
  for(i in names(ts)){
    print(PlotZoneVisitsWithMoving(ts[[i]], points,...) + ggtitle(paste(i)))
  }
  dev.off()
  return(NULL)
}

PlotOFTparamOpto.Multi.PDF <- function(PlotData, BinAnalysisFile, filename, width = 10, height = 4){
  out <- list()
  OFTparam <- names(BinAnalysisFile)[-c(1,2)]
  
  for ( param in OFTparam){
    param2 <- ensym(param)
    ps <- ggplot(data= PlotData, aes(x = bin, y = !!param2))+
      stat_boxplot(geom="errorbar", position = position_dodge(0.8), size = 1, width = 0.5)+
      geom_boxplot(position = position_dodge(0.8),lwd = 1)+
      geom_line(aes(group = VideoName_AnimalID), alpha = 0.5)+
      geom_point(aes(group = VideoName_AnimalID, color = VideoName_AnimalID), size = 3)+
      theme(legend.position = "none")+
      theme_bw()+
      facet_wrap(~`type of opsin`+ FiberLocGroup, nrow = 1)+
...
PlotOFTparamDREADD.Multi.PDF <- function(PlotData, ReportFile, filename, width = 8, height = 4){
  out <- list()
  OFTparam <- names(ReportFile)[-1]
  
  for ( param in OFTparam){
    param2 <- ensym(param)
    ps <- ggplot(data= PlotData, aes(x = Condition, y = !!param2))+
      stat_boxplot(geom="errorbar", position = position_dodge(0.8), size = 1, width = 0.5)+
      geom_boxplot(position = position_dodge(0.8),lwd = 1)+
      geom_line(aes(group = VideoName_AnimalID), alpha = 0.5)+
      geom_point(aes(group = VideoName_AnimalID, color = VideoName_AnimalID), size = 3)+
      theme(legend.position = "none")+
      theme_bw()+
      facet_wrap(~DREADDreceptor, nrow = 1)+
      theme(axis.title.x = element_blank())
...
Opto_BLvsStim_Ttest <- function(DF1, DF2, cols, Opsin, Report = TRUE){
  results <- list()
  Opsin2 <- ensym(Opsin)
  
  for (i in all_of(cols)){
    print(names(DF1)[i])
    if(all(DF1[,i] %>% replace(is.na(.),0) != DF2[,i] %>% replace(is.na(.),0))){
      tryCatch(results[[names(DF1)[i]]][["tTest"]] <- t.test(DF1 %>% filter(`type of opsin` == Opsin) %>% select(i) %>% pull(),
                                                  DF2 %>% filter(`type of opsin` == Opsin) %>% select(i) %>% pull(),
                                                  paired = TRUE),
               error=function(e) e)
      tryCatch(results[[names(DF1)[i]]][["HedgesG"]] <- cohen.d(DF1 %>% filter(`type of opsin` == Opsin) %>% select(i) %>% pull(),
                                                     DF2 %>% filter(`type of opsin` == Opsin) %>% select(i) %>% pull(),
                                                     hedges.correction = TRUE, paired = TRUE, na.rm = TRUE),
               error=function(e) e)
    }
  }
  
  if (Report){
    pValues <- results %>% map("tTest") %>% map ("p.value") %>% unlist() %>% as.data.frame()
    tValues <- results %>% map("tTest") %>% map ("statistic") %>% map("t") %>% unlist() %>% as.data.frame()
    DFs <- results %>% map("tTest") %>% map ("parameter") %>% map("df") %>% unlist() %>% as.data.frame()
    HedgesG <- results %>% map("HedgesG") %>% map ("estimate") %>% unlist() %>% as.data.frame()
    HedgesMag <- results %>%  map("HedgesG") %>% map ("magnitude") %>% unlist() %>% as.data.frame()
    
    CSV <- data.frame(rownames(pValues), pValues, tValues, DFs, HedgesG, HedgesMag)
    tryCatch(names(CSV) <- c("Parameter","tTest_pVal", "tTest_tVal", "tTest_DFs", "HedgesG","HedgesMag"),
             error = function(e) e)
    results[["Report"]] <- CSV
  }
  return(results)
...
DREADD_BLvsStim_Ttest <- function(DF1, DF2 = NULL, cols, DREADD, Report = TRUE){
  results <- list()
  ...
  if(!is.null(DF2)){
    for (i in all_of(cols)){
      print(names(DF1)[i])
      if(all(DF1[,i] %>% replace(is.na(.),0) != DF2[,i] %>% replace(is.na(.),0))){
        results[[names(DF1)[i]]][["tTest"]] <- t.test(DF1 %>% filter(DREADDreceptor == DREADD) %>% select(i) %>% pull(),
                                                      DF2 %>% filter(DREADDreceptor == DREADD) %>% select(i) %>% pull(),
                                                      paired = TRUE)
        results[[names(DF1)[i]]][["HedgesG"]] <- cohen.d(DF1 %>% filter(DREADDreceptor == DREADD) %>% select(i) %>% pull(),
                                                         DF2 %>% filter(DREADDreceptor == DREADD) %>% select(i) %>% pull(),
                                                         hedges.correction = TRUE, paired = TRUE, na.rm = TRUE)
      }
    }
  ...
  if(is.null(DF2)){
    for (i in all_of(cols)){
      print(names(DF1)[i])
      Vec1 <- DF1 %>% filter(DREADDreceptor == DREADD[1]) %>% select(i) %>% pull()
      Vec2 <- DF1 %>% filter(DREADDreceptor == DREADD[2]) %>% select(i) %>% pull()
      if(all(Vec1[1:length(Vec2)] %>% replace(is.na(.),0) != Vec2 %>% replace(is.na(.),0))){
        results[[names(DF1)[i]]][["tTest"]] <- t.test(Vec1,
                                                      Vec2,
                                                      paired = FALSE)
        results[[names(DF1)[i]]][["HedgesG"]] <- cohen.d(Vec1,
                                                         Vec2,
                                                         hedges.correction = TRUE, paired = FALSE, na.rm = TRUE)
      }
    }
  }
